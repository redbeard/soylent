window.HomeCtrl = [ '$scope', '$http', '$timeout', '$products_repository', '$elements_repository', ($scope, $http, $timeout, $products_repository, $elements) ->
  ...
  $scope.recommendation = new Recommendation [
    $elements.Energy.in_quantity("2597 kcal"),
    $elements.Carbohydrates.in_quantity("247.6 g"),
    $elements.Protein.in_quantity("154 g"),
    $elements.Fats.in_quantity("110.1 g"),
    $elements.Sodium.in_quantity("2.4 g"),
    $elements.Potassium.in_quantity("4.5 g"),
    $elements.Chlorine.in_quantity("3.4 g"),
    $elements.Fibre.in_quantity("30 g"),
  ...
  $scope.recipe = new Recipe [
    $products_repository.findBySubstring("Bertolli")[0].in_quantity( new Qty("101ml") ),
    $products_repository.findBySubstring("Fine Powdered Oats")[0].in_quantity( new Qty("135.0g") ),
    $products_repository.findBySubstring("Maltodextrin (BulkNutrients.com.au)")[0].in_quantity( new Qty("165.0g") ),
    $products_repository.findBySubstring("Whey Protein Isolate")[0].in_quantity( new Qty("146g") ),
    $products_repository.findBySubstring("Leucine")[0].in_quantity( new Qty("0g") ),
    $products_repository.findBySubstring("Fibre Cleanse")[0].in_quantity( new Qty("30 g") ),
    $products_repository.findBySubstring("Salt Iodized (Generic)")[0].in_quantity( new Qty("4 g") ),
    ...
    $products_repository.findBySubstring("Monosodium Phosphate")[0].in_quantity( new Qty("3g") ),
    $products_repository.findBySubstring("Creatine")[0].in_quantity( new Qty("5g") ),

    $products_repository.findBySubstring("Opti Men")[0].in_quantity( new Qty("2 count") ),
    $products_repository.findBySubstring("Iron")[0].in_quantity( new Qty("0.25 count") ),
    $products_repository.findBySubstring("Calcium/magnesium")[0].in_quantity( new Qty("0.0 count") ),
